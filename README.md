# 👋 Hi, I'm Ziyad
I'm a software engineer who enjoys learning by building. I've worked on full stack web applications, AI projects, and desktop applications. I like taking on projects that challenge me to learn new technologies and improve my skills. 

## Projects
### RAG Chatbot
A Retrieval-Augmented Generation (RAG) chatbot integrated with a local LLM, containerized with Docker.
- Combines a local large language model with a retrieval pipeline to ground responses in real source material.
- Implemented hybrid search combining BM25 for keyword matching with dense vector embeddings for semantic meaning, improving retrieval relevance over either approach alone.
- Designed to reduce hallucinations by grounding answers in retrieved context rather than relying solely on the model's internal knowledge.
- Fully containerized with Docker for consistent, portable deployment.
`Tech: Python, React, Local LLM, Docker, Vector Database`
***
### CtrlAI
 An AI/ML application that translates real-world body movement into in-game actions in real time.
- A C++ Desktop App.
- Detected movements are translated into keyboard inputs (e.g., pressing spacebar), allowing a player's physical actions to control in-game character movement in real time.
`Tech: C++, Computer Vision, Real-time Video Processing`
***
### HaxBot
A Video game Bot.
Built a reinforcement learning pipeline using Gymnasium and OpenCV to learn policies directly from game frames.
`Tech: Python, AI/ML`
***
### Fashion Store App
- The goal of this project is to develop a fashion store app in Java with a PostgreSQL database for persistence, backed with an extensive JUnit testing suite.
- This implementation follows all software lifecycle steps and agile software engineering strategies.
`Tech: Java, Vue, PostgreSQL, SpringBoot`
***
### Grocery Management System
A GUI Java application for managing a grocery store's operations.
`Tech: Java, JavaFx`
